...
- `LevelDefinition.frameless` (GDD §4.4 excepción): el tablero de un nivel puede prescindir del marco azul en absoluto — `LevelHost` monta el propio `LevelComponent` como `boardBelowFrame` de `XPWindow` directamente, no como `children` (el marco ni se renderiza). Ningún nivel real lo usa por ahora (el nivel 4 lo usó hasta que su texto pasó al marco azul, ver arriba, dejando el tablero mismo sin marco vía `useLevelBoard` en vez de esta ruta) — queda disponible para un futuro nivel de tablero puro sin texto propio en el marco.
