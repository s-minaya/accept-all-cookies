...
4. **Componente** — `Level11.tsx`: consentimiento en el marco (nodo memoizado); vía `useLevelBoard`, el área con el sprite de Sans abajo a la derecha y el `SpeechBubble` (pregunta + botones `No`/`Yes`, `XPButton` con textos de `game.*`); vía `useLevelFooter`, el Disagree rojo → `onLose('failed')`.
...
9. **QA** — ganar las 8; perder en cada posición; Disagree del pie; contador; X a mitad de frase (la música debe volver a su nivel); ráfaga de clics; `skip()`; `paused` a mitad de frase; recarga; 5 anchos; móvil con toques CDP.
...
- **El Disagree del pie es un señuelo funcional** — derrota real: el jugador harto tras seis preguntas tiene una salida a mano, y es la equivocada.
