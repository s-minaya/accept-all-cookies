...
- [x] Implementar `Reel.tsx` (tira triplicada — sube de duplicada tras revisión de Sofía sobre la dirección de giro, ver más abajo —, rAF con velocidad base + desfase, marcador de fila central, transform por custom property).
- [x] Implementar `Level05.tsx`: texto en el marco; tablero (3 rodillos + 3 Stops neutros, deshabilitado=oscurecido, mismo ancho que su rodillo) vía `useLevelBoard`; `coin.mp3` en cada captura (corregido tras revisión de Sofía: sonaba positivo/negativo por símbolo, ahora un único sonido de "moneda" para cualquier captura); `onWin()` con triple Agree, `onLose()` inmediato con triple Disagree (excepción explícita a la rehabilitación, confirmada por Sofía); rehabilitación tras la pausa. Sin `fillHeight`: cada rodillo tiene su propia altura fija, no lo necesita (mismo razonamiento que llevó a quitárselo al nivel 4). Estilo visual retocado tras revisión de Sofía sobre una referencia: borde azul oscuro en cada rodillo, barra que cruza los 3 a media altura, marcador central gris de esquinas redondeadas, botones Agree/Disagree en forma de píldora con margen creciente por breakpoint entre ellos.
...
- [x] ✋ **Checkpoint con Sofía**: tacto de la tragaperras, dirección de giro, estilo visual de los rodillos (revisado contra una referencia, varias rondas de ajuste) y sonido de captura — aprobado ("Está perfecto").
- [x] QA: cubierto por Playwright (partida entera ganando y perdiendo, contador muriendo en plena pausa de rehabilitación, X durante la pausa, Stops deshabilitados no repulsables, recarga a mitad, 5 anchos sin scroll horizontal, toque en emulación iPhone SE) + revisión visual directa de Sofía sobre capturas reales.
