...
- [x] La música suena claramente más baja que los efectos: multiplicador propio sobre su propio máximo, además del volumen de Ajustes. Distinto por dispositivo (`DESKTOP_MUSIC_VOLUME_FACTOR` = 0.5, `MOBILE_MUSIC_VOLUME_FACTOR` = 0.1 en `AudioManager.ts`, elegido por tipo de puntero) — ver decisión y su historial en `002-plan.md`.
