...
3. **Barajado** — tres rondas encadenadas sin pausa: **lenta ~2 s**, **media ~1,5 s**, **rápida ~1 s** (GDD). Los botones nunca desaparecen: intercambian posiciones con movimientos fluidos, estilo trilero. **Durante todo el barajado el input se ignora** (los botones no responden ni por ratón, ni por dedo, ni por teclado).
...
- ⚠️ **Hueco del GDD (pendiente de decisión de Sofía)**: el GDD no dice qué pasa si en la fase *reveal* el jugador pulsa uno de los 11 **Disagree** visibles. Esta spec aplica la regla madre del juego — **pulsar un Disagree es rechazar: derrota inmediata** —, coherente con los niveles 2, 3 y 7. Alternativa posible: que no haga nada (el nivel solo empieza al pulsar el Agree). Se implementa la primera salvo indicación contraria, y se anota en el GDD.
...
### Fases y mecánica
- [ ] Reveal: 11 Disagree + 1 Agree, posición del Agree aleatoria en cada partida (test con semillas distintas).
- [ ] Pulsar el Agree dispara el flip de los 12 a la vez y todos quedan en estilo neutro con `???`; ninguno delata su identidad (ni visualmente ni en el DOM).
- [ ] Tres rondas de barajado encadenadas con sus duraciones (~2 / ~1,5 / ~1 s); los botones intercambian posiciones sin desaparecer.
- [ ] Durante el flip y las tres rondas, **ningún** input tiene efecto (ratón, dedo o teclado); tras la última, la elección vuelve a estar disponible.
- [ ] La lógica de permutación es pura y determinista: dada una semilla, la posición final del Agree es reproducible y coincide con la que el componente considera ganadora (test que simula las tres rondas y comprueba la trazabilidad del Agree).

### Victoria y derrota
- [ ] Elegir el botón que era Agree → victoria con el flujo estándar y la categoría correcta; elegir otro → derrota estándar.
- [ ] Pulsar un Disagree en la fase reveal se comporta según la decisión de Sofía (por defecto: derrota).
- [ ] Contador a 0 y X pierden en cualquier fase, incluido a mitad de barajado.

### Integración y calidad
- [ ] Hueco 8 sustituido con chunk propio (sin matter.js); `levels.8.*` en ambos diccionarios; PRNG compartido reutilizado.
- [ ] Cuadrícula 4×3 en md+ y 3×4 en xs/sm, botones ≥ 44 px, sin scroll en 375 px; los recorridos del barajado se ven completos dentro del área.
- [ ] El barajado no re-renderiza el árbol por frame (posiciones por custom property / transiciones CSS, patrón as-built de las 009–011).
- [ ] `paused` congela flip, barajado e input y reanuda sin saltos; recarga a mitad (reveal nuevo, contador restaurado) y con desenlace pendiente (modal).
- [ ] Cleanup total al desmontar (timers de fase, listeners) + test de no-fugas.
- [ ] Partida entera ganando y perdiendo; dedo y ratón; 5 anchos; móvil real vía Pages.
- [ ] ✋ Checkpoint de Sofía: legibilidad del barajado (que se pueda seguir el botón con la vista y que las tres velocidades se noten).
