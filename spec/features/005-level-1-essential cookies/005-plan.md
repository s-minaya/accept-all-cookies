...
3. **Componente** — `src/levels/level01/Level01.tsx`: texto de consentimiento (`levels.1.*`), zona de botones con hueco reservado para Agree y `XPButton` Disagree; al pulsar Disagree pasa a `errorDialog` y renderiza el `XPDialog` de error; OK → `onRestart()`. Pulsar Agree → `onWin()`.
4. **Estilos** — `level01.scss` en BEM (`level01__buttons`, `level01__agree-slot`…), tokens compartidos, sin colores nuevos.
